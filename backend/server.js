...
app.get('/api/dashboard/estatisticas', async (req, res) => {
  try {
    ...
    console.log('✅ Estatísticas do dashboard carregadas:', {
      livros: totalLivros[0].total,
      emprestimos: emprestimosAtivos[0].total,
      reservas: reservasAtivas[0].total,
      atrasos: emprestimosAtraso[0].total
    });
    ...
    console.error('❌ Erro em /api/dashboard/estatisticas:', error);
    res.status(500).json({
      success: false,
...
// Empréstimos recentes para o dashboard
app.get('/api/dashboard/emprestimos-recentes', async (req, res) => {
  try {
    const connection = await mysqlPool.getConnection();
    ...
    console.error('❌ Erro em /api/dashboard/emprestimos-recentes:', error);
    res.status(500).json({
      success: false,
      error: 'Erro ao carregar empréstimos recentes',
      message: error.message
